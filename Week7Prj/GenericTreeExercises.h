...
static void treeFactory(GenericTree<int> &tree) {
  ...
  auto B = A->addChild(8);
  auto C = A->addChild(15);
  auto D = B->addChild(16);
  auto E = B->addChild(23);
  auto F = D->addChild(42);
}
...
// (You do NOT need to edit this function.)
template <typename N> int countNullChildrenRecursive(N *subtreeRoot) {
...
// TreeNode inner type belonging to some type of GenericTree.
// (You do NOT need to edit this function.)
template <typename N> int countNullChildrenIterative(N *subtreeRoot) {
...
template <typename T> std::vector<T> traverseLevels(GenericTree<T> &tree) {
